...
{
    // commit: haizhu.shao 2016-11-29 12:42
    // 如果这个时候b在使用，那么也是empty？
    // commit: haizhu.shao 2016-11-29 12:53
    // 没有问题，因为数据是先写入a_start，然后才会写入b
...
{
    // commit: haizhu.shao 2016-11-29
    // 如果这个时候b_inuse为0，而a_start - b_end > size，那么难道也不能通过修改b_inuse来使用这块空间吗？
    // 明白了，bipbuf_request的调用是在bipbuf_push或者bipbuf_offer之后，这两个函数都会调用__check_for_switch_to_b.
    // 如果a_start - b_end > size，那么假如__check_for_switch_to_b没有修改b_inuse，这个时候a_end - a_start也会 > size
